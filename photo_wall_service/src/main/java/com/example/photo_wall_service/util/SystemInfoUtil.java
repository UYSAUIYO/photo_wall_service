...
public class SystemInfoUtil {
    ...
    public static JSONObject getSysInfo() throws UnknownHostException {
        ...
        sysInfo.put("computerName", computerName);
        sysInfo.put("computerIp", computerIp);
        sysInfo.put("osName", osName);
        sysInfo.put("osArch", osArch);
        sysInfo.put("osVersion", osVersion);
        sysInfo.put("userDir", userDir);

        return sysInfo;
    }

    //所有系统信息
    public static JSONObject getInfo() throws InterruptedException, ExecutionException {
        ...
        // 提交任务并获取Future对象
        Future<JSONObject> cpuInfoFuture = executor.submit(SystemInfoUtil::getCpuInfo);
        Future<JSONObject> jvmInfoFuture = executor.submit(SystemInfoUtil::getJvmInfo);
        Future<JSONObject> memInfoFuture = executor.submit(SystemInfoUtil::getMemInfo);
        Future<JSONArray> sysFileInfoFuture = executor.submit(SystemInfoUtil::getSysFileInfo);
        Future<JSONObject> sysInfoFuture = executor.submit(SystemInfoUtil::getSysInfo);
